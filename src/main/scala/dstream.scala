...
object dstream {
  ...
  def writingData(streamToWrite: MapWithStateDStream[(Timestamp, Timestamp, String), (Events, Int), Int, ((Timestamp, Timestamp, String), (Events, Int))], sparkSession: SparkSession) = {
    streamToWrite.foreachRDD(rdd => {
  ...
  def prepareRDDToWrite(stream: InputDStream[ConsumerRecord[String, String]]) = {
    stream
      .map(_.value)
      .map(x => parseString(x))
      .filter(_.nonEmpty)
  ...
  def createSparkEnvironment = {
    val conf = new SparkConf()
      .setMaster("local[*]")
      .setAppName("DStream-bot-detection")
  ...
  private def accumulateMappingFunction (key: (Timestamp, Timestamp, String),
